Sub Kaufkandidaten()
    '
    ' Zeigt die besten Kaufkandidaten sortiert nach
    ' "Differenz" (Spalte R) in absteigender Reihenfolge.
    ' Nur echte Datenzeilen (7 bis 82) mit positiver Differenz werden angezeigt.
    ' Alle Aktien werden beruecksichtigt, egal ob derzeit gehalten oder nicht.
    ...
    For i = FIRST_ROW To LAST_ROW
        ...
        Dim val As Variant
        val = ws.Cells(i, 18).Value  ' Spalte R = "Differenz"
...
NextRow1:
    ...
    If count = 0 Then
        MsgBox "Keine Kaufkandidaten gefunden." & vbCrLf & _
               "Pruefe, ob Spalte R Werte enthaelt und ob die Formeln aktuell sind." & vbCrLf & _
               "(Tipp: Strg+Alt+F9 druecken, um alle Formeln neu zu berechnen)", _
               vbInformation, "Kaufkandidaten"
        Exit Sub
    End If
    
    ' Arrays dimensionieren
    ReDim titel(1 To count)
    ReDim diff(1 To count)
...
NextRow2:
    ...
    ' Bubble Sort absteigend nach Differenz
    Dim j As Long
    ...
    For i = 1 To count - 1
        For j = 1 To count - i
            If diff(j) < diff(j + 1) Then
                tmpStr = titel(j): titel(j) = titel(j + 1): titel(j + 1) = tmpStr
                tmpDbl = diff(j): diff(j) = diff(j + 1): diff(j + 1) = tmpDbl
    ...
    ' Pruefen, ob Sheet "Kaufliste" schon existiert
    Dim wsOut As Worksheet
    On Error Resume Next
    Set wsOut = ThisWorkbook.Sheets("Kaufliste")
    On Error GoTo 0
    
    If wsOut Is Nothing Then
        Set wsOut = ThisWorkbook.Sheets.Add(After:=ThisWorkbook.Sheets(ThisWorkbook.Sheets.count))
        wsOut.Name = "Kaufliste"
    Else
        wsOut.Cells.Clear
    End If
    
    ' Header
    wsOut.Cells(1, 1).Value = "Kaufkandidaten (Stand: " & Format(Now, "dd.mm.yyyy hh:nn") & ")"
    ...
    wsOut.Cells(3, 5).Value = "Differenz (EUR)"
    ...
    For i = 1 To count
        ...
        wsOut.Cells(i + 3, 5).Value = diff(i)
        wsOut.Cells(i + 3, 5).NumberFormat = "#,##0"
        
        ' Stueckzahl fuer EUR 8.000
        If kurs(i) > 0 Then
            stkKauf = Application.WorksheetFunction.RoundUp(8000 / kurs(i), 0)
        Else
            stkKauf = 0
        End If
        wsOut.Cells(i + 3, 6).Value = stkKauf
    Next i
    ...
    MsgBox count & " Kaufkandidaten gefunden." & vbCrLf & _
           "Ergebnis steht im Sheet 'Kaufliste'.", _
           vbInformation, "Kaufkandidaten"
End Sub
